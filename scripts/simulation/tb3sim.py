...
def loggingcsv(data):
    with open("/home/dryan/Desktop/SimulasiKamis/Tb3_1.csv", "w") as file:
        # Write headers
...
def create_moving_marker(x_start, y_start, x_end, y_end, velocity=0.22):
    ...
    marker_pub = rospy.Publisher('Turtlebot', Marker, queue_size=10)
...
if __name__ == '__main__':
    try:
        ...
        # WP Single TB + semua waypoint
        create_moving_marker(0, 0, 5.5, -0.136)
        create_moving_marker(last_x, last_y, 5.0, -0.793)

        create_moving_marker(last_x, last_y, -0.0229, -0.738)
        create_moving_marker(last_x, last_y, -0.132, -1.324)

        create_moving_marker(last_x, last_y, 5, -1.6)
        create_moving_marker(last_x, last_y, 4.5, -2.876)

        create_moving_marker(last_x, last_y, 3.446, -2.724)
        create_moving_marker(last_x, last_y, 3.302, -3.684)

        create_moving_marker(last_x, last_y, 3.987, -3.758)
        create_moving_marker(last_x, last_y, 3.438, -5.048)

        create_moving_marker(last_x, last_y, -0.275, -2.923)
        create_moving_marker(last_x, last_y, 2.112, -2.950)

        create_moving_marker(last_x, last_y, 2.136, -2.296)
        create_moving_marker(last_x, last_y, 0.038, -1.974)
