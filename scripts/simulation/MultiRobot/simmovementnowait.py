...
def choose_new_position(coordinates, visited_positions, min_distance=3.0):
    potential_positions = [
        p for p in coordinates if p not in visited_positions
    ]

    # Prioritize positions with minimum distance
    prioritized_positions = [
        p for p in potential_positions
        if all(distance_between_points(p, vp) > min_distance for vp in visited_positions)
    ]
    ...
    return random.choice(potential_positions) if potential_positions else None

def robot_routine(robot_namespace, coordinates, initial_position):
    ...
    while not rospy.is_shutdown():
        new_position = choose_new_position(coordinates, visited_positions)
        if new_position is None:
            rospy.loginfo('{} has visited all positions. Exiting.'.format(robot_namespace))
            break

        yaw = calculate_yaw(position[0], position[1], new_position[0], new_position[1])

        send_goal(robot_namespace, new_position[0], new_position[1], yaw)

        if wait_for_goal_reached(robot_namespace, new_position):
...
if __name__ == '__main__':
    try:
        ...
        # Start separate threads for each robot with their own visited positions
        tb3_0_thread = threading.Thread(target=robot_routine, args=('tb3_0', coordinates, tb3_0_position))
        tb3_1_thread = threading.Thread(target=robot_routine, args=('tb3_1', coordinates, tb3_1_position))
